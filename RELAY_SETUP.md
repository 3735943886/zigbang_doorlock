...
**relay가 먼저, 락 리셋은 나중에**입니다 — 6~7번(리셋→재프로비저닝)은 짝이라 바로 이어서
합니다. 락을 미리 리셋해놓고 relay 준비(1~5번, 도메인/인증서 발급 등 시간이 걸릴 수 있는
작업)를 하면, 락이 SoftAP를 띄운 채로 오래 방치되어 버립니다 — relay가 실제로 응답 가능한
상태까지 다 만들어놓은 뒤에 리셋→재프로비저닝을 곧바로 이어서 하세요.
